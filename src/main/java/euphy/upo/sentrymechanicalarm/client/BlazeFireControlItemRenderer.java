...
public class BlazeFireControlItemRenderer extends BlockEntityWithoutLevelRenderer {
   private static final ResourceLocation SCOPE_TEXTURE = ResourceLocation.fromNamespaceAndPath(
      "sentrymechanicalarm", "textures/item/blaze_fire_control_scope.png"
   );
   ...
   public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack ms, MultiBufferSource buffer, int packedLight, int packedOverlay) {
      ...
      if (isUsing && displayContext != ItemDisplayContext.GUI) {
         ...
         BufferBuilder bb = RenderSystem.renderThreadTesselator().begin(Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
         ...
         BlockState state = ((BlazeFireControlBlock)SentryRegistry.BLAZE_FIRE_CONTROL.get()).defaultBlockState();
         ms.pushPose();
         Minecraft.getInstance().getBlockRenderer().renderSingleBlock(state, ms, buffer, packedLight, packedOverlay);
         float time = AnimationTickHolder.getRenderTime();
         float offset = Mth.sin(time / 16.0F % (float) (Math.PI * 2)) / 16.0F;
         if (SentryPartialModels.BLAZE_FIRE_CONTROLLER_HEAD.get() != null) {
            ...
            if (headBuffer != null) {
               ((SuperByteBuffer)((SuperByteBuffer)headBuffer.translate(0.0, (double)offset, 0.0)).rotateCentered(0.0F, Direction.UP))
                  .light(15728880)
         ...
         if (SentryPartialModels.RING.get() != null) {
            ...
            if (ringBuffer != null) {
               ((SuperByteBuffer)((SuperByteBuffer)ringBuffer.translate(0.0, (double)offset, 0.0)).rotateCentered(0.0F, Direction.UP))
                  .light(15728880)
                  .renderInto(ms, buffer.getBuffer(RenderType.cutout()));
            }
         }
