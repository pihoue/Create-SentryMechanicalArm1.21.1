...
public class FireControlInteractionBehaviour extends MovingInteractionBehaviour {
   public boolean handlePlayerInteraction(Player player, InteractionHand activeHand, BlockPos localPos, AbstractContraptionEntity contraptionEntity) {
      ...
      for (MutablePair<StructureBlockInfo, MovementContext> actor : contraption.getActors()) {
         if (((StructureBlockInfo)actor.left).pos().equals(localPos)) {
            context = (MovementContext)actor.right;
            break;
         }
      }
      ...
      if (context == null) {
         ...
         if (context.data.contains("SyncedInventory")) {
            inventory.deserializeNBT(player.level().registryAccess(), context.data.getCompound("SyncedInventory"));
         }
         ...
         if (success) {
            ...
            context.data.put("SyncedInventory", inventory.serializeNBT(registries));
            ...
            if (!player.level().isClientSide) {
               ...
               context.blockEntityData.put("Inventory", inventory.serializeNBT(registries));
            }
