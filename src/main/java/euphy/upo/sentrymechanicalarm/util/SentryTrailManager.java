...
public class SentryTrailManager {
   private static final List<SentryTrailManager.Tracer> tracers = new ArrayList<>();

   public static void addTracer(Vec3 start, Vec3 direction, double speed, double length, double distance) {
      tracers.add(new SentryTrailManager.Tracer(start, direction, speed, length, distance));
   }

   @SubscribeEvent
   public static void onClientTick(Post event) {
      if (!Minecraft.getInstance().isPaused()) {
         Iterator<SentryTrailManager.Tracer> it = tracers.iterator();

         while (it.hasNext()) {
            SentryTrailManager.Tracer t = it.next();
            t.traveled = t.traveled + t.speed;
   ...
   public static void onRenderWorld(RenderLevelStageEvent event) {
      if (event.getStage() == Stage.AFTER_TRANSLUCENT_BLOCKS) {
         if (!tracers.isEmpty()) {
            ...
            for (SentryTrailManager.Tracer t : tracers) {
               double currentDist = t.traveled + t.speed * (double)partialTicks;
               double headDist = Math.min(currentDist, t.maxDistance);
               double tailDist = Math.max(0.0, currentDist - t.length);
               if (!(headDist <= 0.0) && !(tailDist >= t.maxDistance) && !(tailDist >= headDist)) {
   ...
   private static void renderTracerAsBillboard(VertexConsumer builder, Matrix4f matrix, Vec3 start, Vec3 end, float width, Vector3f camPos) {
      ...
      float u = 0.0F;
      float v = 1.0F;
      builder.addVertex(matrix, v1.x, v1.y, v1.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v2.x, v2.y, v2.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v3.x, v3.y, v3.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v4.x, v4.y, v4.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v4.x, v4.y, v4.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v3.x, v3.y, v3.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v2.x, v2.y, v2.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
      builder.addVertex(matrix, v1.x, v1.y, v1.z).setColor(r, g, b, a).setUv(u, v).setLight(light);
